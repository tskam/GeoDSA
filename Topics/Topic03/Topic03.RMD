...
.pull-left[
.large[
In this exercise, URA's Master Plan 2014 Subzone Boundary (i.e. MP14_SUBZONE_WEB_PL) will be used.

- Show the code chunk used to import the geospatial data into R as an sf data.frame.  Name the sf data.frame *mpsz*.
]]
...
.pull-left[
.large[
Perform georelation join by using planning sub-zone name as the unique identifier.
]
]
--
.pull-right[
...
```{r}
mpsz_popdata2020 <- left_join(mpsz,
                              popdata2020,
                              by = c("SUBZONE_N" = "SZ"))
```
]
...
```{r, eval=FALSE}
tm_shape(mpsz_popdata2020[mpsz_popdata2020$REGION_N=="CENTRAL REGION", ]) + #<<
  tm_fill("DEPENDENCY", 
          style = "quantile", 
          palette = "Blues", 
          legend.hist = TRUE, 
